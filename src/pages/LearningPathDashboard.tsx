...
import { PAGE_MODULES, PAGE_QUIZ_QUESTIONS, PAGE_LINKS } from "./LearningPathData";

const studySteps = [
  {
    title: "Orient",
    text: "See where you are, what is complete, and what comes next.",
    icon: "explore",
  },
  {
    title: "Learn",
    text: "Open a module and focus on the core concept without clutter.",
    icon: "menu_book",
  },
  {
    title: "Prove",
    text: "Use the quiz to confirm the idea stuck before moving on.",
    icon: "quiz",
  },
];
...
export default function ModelUNAcademy() {
  ...
  const currentPageId = PAGE_LINKS.some((page) => page.id === pageId) ? (pageId as string) : PAGE_LINKS[0].id;
  const currentPage = PAGE_LINKS.find((page) => page.id === currentPageId) || PAGE_LINKS[0];
  const otherPageLinks = PAGE_LINKS.filter((page) => page.id !== currentPageId);
  const currentModules = PAGE_MODULES[currentPageId as keyof typeof PAGE_MODULES] || PAGE_MODULES["page-1"];
  const currentPageQuiz = PAGE_QUIZ_QUESTIONS[currentPageId as keyof typeof PAGE_QUIZ_QUESTIONS] || PAGE_QUIZ_QUESTIONS["page-1"];
  ...
  const activeModule = currentModules.find((m) => m.id === activeModuleId) || currentModules[0];
  const nextModule = currentModules.find((m) => m.id === Math.min(activeModuleId + 1, totalModules));
  const progressPercent = Math.round((activeModuleId / totalModules) * 100);
  const completedModules = Math.max(activeModuleId - 1, 0);
  const remainingModules = Math.max(totalModules - activeModuleId, 0);
  ...
  const activeQuizQuestions = currentPageQuiz[activeModuleId as keyof typeof currentPageQuiz] || [];
  const currentQuestion = useMemo(() => {
    return activeQuizQuestions[currentQuizQuestion];
  }, [activeQuizQuestions, currentQuizQuestion]);
  ...
  const handleFinishStudy = () => {
    ...
    if (activeModuleId < totalModules) {
  ...
  const studyScore = activeModuleId === 1 ? "Getting started" : activeModuleId < totalModules ? "Building momentum" : "Final checkpoint";
  const hasQuiz = activeQuizQuestions.length > 0;
  const nextActionLabel = viewMode === "journey" ? "Open current module" : showQuiz ? "Continue knowledge check" : quizComplete ? "Finish and move forward" : hasQuiz ? "Begin knowledge check" : "Module complete";
    ...
    <div className="dark">
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 768px) {
          ...
          .module-title { font-size: 1.6rem !important; }
        }
      `}</style>
      ...
      >
        <div className="panel" style={{ borderRadius: 20, padding: 18, marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: 10, fontWeight: 900, letterSpacing: "0.2em", textTransform: "uppercase", color: "#0ea5e9", marginBottom: 6 }}>
                Learning path
              </p>
          ...
          <p style={{ color: "#cbd5e1", lineHeight: 1.55, fontSize: 14 }}>
            Move between the four pages without changing the learning layout.
          </p>
        </div>

        <div className="panel-soft" style={{ borderRadius: 20, padding: 18, marginBottom: 16 }}>
          <p style={{ fontSize: 10, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.2em", color: "#0ea5e9", marginBottom: 12 }}>
            Other pages
          </p>
            ...
            {otherPageLinks.map((page) => (
              <button
                ...
                onMouseOver={(e) => {
                  ...
                  e.currentTarget.style.background = "rgba(7,89,133,0.22)";
                }}
                onMouseOut={(e) => {
                  e.currentTarget.style.transform = "translateX(0)";
                  e.currentTarget.style.borderColor = "rgba(14,165,233,0.12)";
                  e.currentTarget.style.background = "rgba(15,23,42,0.86)";
                }}
              >
      ...
      >
        ...
        <div className="fade-in" style={{ position: "relative", zIndex: 2 }}>
          {viewMode === "journey" && (
            <div style={{ width: "100%" }}>
              <section className="page-pad" style={{ padding: "2rem 2rem 1rem" }}>
                <div className="panel" style={{ borderRadius: 28, padding: "1.5rem", overflow: "hidden" }}>
                  <div className="stack-mobile" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 24 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
                        <Pill tone="blue">{currentPage.label}</Pill>
                        <Pill tone="blue">Module {activeModuleId} of {totalModules}</Pill>
                        <Pill tone="green">{progressPercent}% ready</Pill>
                      </div>
                      <p style={{ fontSize: 11, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.24em", color: "#0ea5e9", marginBottom: 10 }}>
                        Learning journey
                      </p>
                      <h2 className="hero-title" style={{ fontFamily: "'Noto Serif', serif", fontSize: 56, fontStyle: "italic", lineHeight: 1.02, color: "#fff", maxWidth: 900, marginBottom: 12 }}>
                        Learn MUN in a way that feels clear, guided, and motivating.
                      </h2>
                      <p style={{ maxWidth: 780, fontSize: 16, lineHeight: 1.7, color: "#cbd5e1" }}>
                        {learningSignals[activeModuleId % learningSignals.length]}
                      ...
                      <div className="panel-soft" style={{ borderRadius: 20, padding: 16 }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
                          <div>
                            ...
                            <p style={{ color: "#fff", fontFamily: "'Noto Serif', serif", fontSize: 18 }}>{getDiplomaticRank(activeModuleId)}</p>
                          </div>
                        ...
                        <p style={{ color: "#94a3b8", fontSize: 13, lineHeight: 1.5 }}>
                          {completedModules} completed · {remainingModules} ahead · next up: {nextModule?.title}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <section className="page-pad" style={{ padding: "0 2rem 2rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(12, minmax(0, 1fr))", gap: 20 }}>
                  <div style={{ gridColumn: "span 4" }}>
                    <div className="panel-soft" style={{ borderRadius: 24, padding: 20, marginBottom: 16 }}>
                      ...
                      <p style={{ color: "#cbd5e1", lineHeight: 1.7, marginBottom: 18 }}>{activeModule.shortDesc}</p>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12 }}>
                        <div style={{ padding: 14, borderRadius: 18, background: "rgba(15,23,42,0.84)", border: "1px solid rgba(14,165,233,0.12)" }}>
                          ...
                          <p style={{ color: "#fff", fontWeight: 800 }}>Module 0{activeModule.id}</p>
                        </div>
                        <div style={{ padding: 14, borderRadius: 18, background: "rgba(15,23,42,0.84)", border: "1px solid rgba(14,165,233,0.12)" }}>
                    ...
                    <div className="panel" style={{ borderRadius: 28, padding: 22, marginBottom: 18 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                        {currentModules.map((module) => {
                          ...
                          const isUpcoming = module.id > activeModuleId;
                          ...
                          return (
                            <button
                              ...
                              style={{
                                ...
                                gap: 14,
                                background: isActive
                                  ? "linear-gradient(145deg, rgba(7,89,133,0.36), rgba(15,23,42,0.96))"
                                  : isCompleted
                                    ? "linear-gradient(145deg, rgba(15,118,110,0.20), rgba(15,23,42,0.92))"
                                    : "rgba(15,23,42,0.84)",
                              ...
                              onMouseOver={(e) => {
                                ...
                                if (!isActive) e.currentTarget.style.borderColor = "rgba(14,165,233,0.24)";
                              }}
                              onMouseOut={(e) => {
                                e.currentTarget.style.transform = "translateY(0)";
                                if (!isActive) e.currentTarget.style.borderColor = isCompleted ? "rgba(34,197,94,0.18)" : "rgba(14,165,233,0.10)";
                              }}
                            >
                              <div style={{ width: 54, height: 54, borderRadius: 18, flexShrink: 0, display: "grid", placeItems: "center", background: isActive ? "rgba(14,165,233,0.16)" : isCompleted ? "rgba(34,197,94,0.14)" : "rgba(7,89,133,0.12)", border: "1px solid rgba(14,165,233,0.12)" }}>
                                <span className="material-symbols-outlined" style={{ color: isActive ? "#7dd3fc" : isCompleted ? "#86efac" : "#94a3b8", fontSize: 24, fontVariationSettings: isActive ? "'FILL' 1" : "'FILL' 0" }}>{module.icon}</span>
                              </div>

                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, marginBottom: 8 }}>
                                  <span style={{ fontSize: 10, fontWeight: 900, letterSpacing: "0.16em", textTransform: "uppercase", color: isActive ? "#7dd3fc" : isCompleted ? "#86efac" : "#94a3b8" }}>
                                    Module 0{module.id}
                                  </span>
                                ...
                                <h4 style={{ fontFamily: "'Noto Serif', serif", color: "#fff", fontSize: 20, marginBottom: 6 }}>{module.title}</h4>
                                <p style={{ color: "#cbd5e1", lineHeight: 1.6, fontSize: 14 }}>{module.shortDesc}</p>
                              </div>

                              <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 8, color: isActive ? "#7dd3fc" : "#64748b" }}>
                                <span style={{ fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em" }}>
                                  Open
                                </span>
            ...
            <div style={{ width: "100%" }}>
              <section className="page-pad" style={{ padding: "2rem 2rem 1rem" }}>
                <div className="panel" style={{ borderRadius: 28, padding: "1.4rem", position: "sticky", top: 86, zIndex: 8 }}>
                  <div className="stack-mobile" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 18 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 14, minWidth: 0 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 8 }}>
                          <Pill tone="blue">Module 0{activeModule.id}</Pill>
                          <Pill tone="blue">{progressPercent}% ready</Pill>
                          <Pill tone="green">{studyScore}</Pill>
                        </div>
                        <p style={{ fontSize: 10, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.22em", color: "#0ea5e9", marginBottom: 6 }}>
                          Focus mode
                        </p>
                        <h2 className="module-title" style={{ fontFamily: "'Noto Serif', serif", fontSize: 30, color: "#fff" }}>{activeModule.title}</h2>
                      </div>
                    </div>
                    ...
                    >
                      <span className="material-symbols-outlined" style={{ color: "#0ea5e9" }}>refresh</span>
                      Reset
                    </button>
                  </div>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(12, minmax(0, 1fr))", gap: 20 }}>
                  <aside style={{ gridColumn: "span 4", display: "flex", flexDirection: "column", gap: 16 }}>
                    <div className="panel-soft" style={{ borderRadius: 24, padding: 20 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12 }}>
                        <div style={{ padding: 14, borderRadius: 18, background: "rgba(15,23,42,0.84)", border: "1px solid rgba(14,165,233,0.12)" }}>
                          ...
                          <p style={{ color: "#fff", fontWeight: 800 }}>{getDiplomaticRank(activeModuleId)}</p>
                        </div>
                        <div style={{ padding: 14, borderRadius: 18, background: "rgba(15,23,42,0.84)", border: "1px solid rgba(14,165,233,0.12)" }}>
                    ...
                    <div className="panel-soft" style={{ borderRadius: 24, padding: 20 }}>
                      ...
                      <p style={{ color: "#94a3b8", fontSize: 13, lineHeight: 1.6 }}>
                        {completedModules} completed · {remainingModules} remaining.
                      </p>
                    </div>
                  ...
                  <div style={{ gridColumn: "span 8", display: "flex", flexDirection: "column", gap: 18 }}>
                    <div className="panel" style={{ borderRadius: 28, padding: 22 }}>
                      ...
                      <div style={{ padding: 16, borderRadius: 20, background: "rgba(7,89,133,0.16)", border: "1px solid rgba(14,165,233,0.12)", marginBottom: 18, display: "flex", alignItems: "flex-start", gap: 12 }}>
                    ...
                    {showQuiz && !quizComplete && currentQuestion && (
                      <div className="panel-soft" style={{ borderRadius: 28, padding: 22 }}>
                        ...
                        <p style={{ fontSize: 20, fontWeight: 800, color: "#fff", marginBottom: 18, lineHeight: 1.45 }}>{currentQuestion.q}</p>
                              ...
                              <button
                                ...
                                onMouseOver={(e) => {
                                  if (!showFeedback) {
                                    ...
                                    e.currentTarget.style.backgroundColor = "rgba(7,89,133,0.45)";
                                    e.currentTarget.style.borderColor = "rgba(14,165,233,0.34)";
                                  }
                                }}
                                onMouseOut={(e) => {
                                  if (!showFeedback) {
                                    e.currentTarget.style.transform = "translateY(0)";
                                    e.currentTarget.style.backgroundColor = "rgba(15,23,42,0.74)";
                                    e.currentTarget.style.borderColor = "rgba(14,165,233,0.18)";
                                  }
                                }}
                              >
                                <span style={{ width: 28, height: 28, borderRadius: 999, display: "grid", placeItems: "center", background: "rgba(14,165,233,0.12)", border: "1px solid rgba(14,165,233,0.16)", flexShrink: 0, color: "#7dd3fc", fontSize: 12, fontWeight: 900 }}>
                                  {idx + 1}
                                </span>
                    ...
                    {quizComplete && (
                      <div className="panel-soft" style={{ borderRadius: 28, padding: 22 }}>
                        <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 16 }}>
                          ...
                          <div>
                            <p style={{ fontSize: 10, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.2em", color: quizPassed ? "#22c55e" : "#ef4444", marginBottom: 6 }}>
                              {quizPassed ? "Checkpoint complete" : "Checkpoint not passed"}
                            </p>
                            <h4 style={{ fontFamily: "'Noto Serif', serif", fontSize: 24, color: "#fff", marginBottom: 8 }}>
                              {quizPassed ? "Nice work — keep the momentum going." : "You need 2 correct answers to unlock the lesson."}
                            </h4>
                            <p style={{ color: "#cbd5e1", lineHeight: 1.7 }}>
                              You scored {quizAnswers.reduce((acc, ans, idx) => acc + (ans === activeQuizQuestions[idx]?.correct ? 1 : 0), 0)}/{activeQuizQuestions.length} correct.
                            </p>
                          </div>
                        </div>
                        ...
                        {quizPassed ? (
                          ...
                          >
                            <span>{activeModule.id < totalModules ? "Continue to next module" : "Return to journey"}</span>
                            <span className="material-symbols-outlined">arrow_forward</span>
                          </button>
                        ) : (
                          <button
